...
class WarehouseIOController extends Controller
{
    ...
    public function update(Request $request, WarehouseIO $warehouseIO)
    {
        ...
        foreach ($packages as $package) {
            ...
            foreach ($resultPkgs as $resultPkg) {
                if ($package[1] === $resultPkg->sku) {
                    ...
                    if ($resultPkg->real_quantity !== null) {
                        if (intval($request->type) === 0) {
                            ...
                            // và nhưng khi nhập/xuất hàng không thể biết số lượng của nó ra sao
                            // vì vậy không thể chắc khi trừ đi nó sẽ luôn về 0, 
                            // nên vẫn cần phải trừ đi cộng lại.
                            if (!$hold) {
                        ...
                        // Nếu là lần đầu cập nhật khi real_quantity = null 
                        // Riêng với nhập xuất kho, khi real_quantity = null đồng nghĩa với phiếu nhập/xuất
                        // mới được tạo và chưa tác động gì đến quantity trong database cả, vì vậy có thể
                        // cập nhật trực tiếp mà không cần trừ/cộng giá trị quantity cũ
                        if (intval($request->type) === 0) {
                            $colorSize->quantity += $package[4];
            ...
            // Nếu request package không nằm trong dữ liệu truy vấn database trả về thì thêm
            if (!$existed) {
                Package::create([
                    'name' => $package[0],
                    'sku' => $package[1],
                ...
                // Xử lý trường hợp khi thêm packages mới vào, 
                // cập nhật quantity (color_sizes và warehouse_products)
                // Xử lý tương tự như trường hợp real_quantity = null
                if (intval($request->type) === 0) {
                    $colorSize->quantity += $package[4];
        ...
        foreach ($delPkgs as $delPkg) {
            ...
            // Xử lý trường hợp sau khi xóa package
            // Cập nhật lại quantity của color_sizes và warehouse_products
        }
